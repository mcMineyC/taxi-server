...
//  await x.modify(
//    (xy) => {
//      if(xy.roles == undefined) xy.roles = ["view", "add", "dj", "admin"];
//      if(!xy.roles.includes("view")) xy.roles.push("view");
//      if(!xy.roles.includes("add")) xy.roles.push("add");
//      if(!xy.roles.includes("admin")) xy.roles.push("admin");
//      if(!xy.roles.includes("dj")) xy.roles.push("dj");
//      if(!xy.roles.includes("recruiter")) xy.roles.push("recruiter");
//      if(xy.loginName == "jedi") xy.roles.push("sudoadmin");
//      if(xy.loginName == "connor") xy.roles.push("sudoadmin");
//      if(xy.loginName == "abby") xy.roles.push("sudoadmin");
//      console.log("Updating roles", xy.displayName, "\t", xy.roles);
//      return xy;
//    }
//  )
//);

//var data = await ts.searchArtist("2 mello")
//console.table(data);
