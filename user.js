...
while (true) {
  ...
  switch (act) {
    case "create":
      ...
      var password = readline.question("Password (optional): ", {
        hideEchoBack: true,
      });
      var roles = readline.question(
        "Roles (comma separated, default view,admin,dj,add,recruiter can be sudoadmin): ",
      );
      if (roles == "") roles = "view,admin,dj,add,recruiter";
      roles = roles.split(",");
      const existingUser = await db
        .collection("auth")
        .findOne({ loginName: name });
      if (existingUser) {
